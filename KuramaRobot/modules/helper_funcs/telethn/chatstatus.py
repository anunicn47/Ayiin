from KuramaRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from KuramaRobot import DRAGONS
...
async def kurama_is_admin(chat_id: int):
    status = False
    kurama = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins,
    ):
        if kurama.id == user.id:
            status = True
            break
    return status
