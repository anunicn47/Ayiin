...
def protecc(update: Update, context: CallbackContext):
    if os.path.isfile("okgoogle.png"):
        os.remove("okgoogle.png")
    ...
    if reply:
        if reply.sticker:
            ...
            msg.reply_text("Reply to an waifu or husbando image .")
            return
        image_file = bot.get_file(file_id)
        image_file.download(imagename)
        if args:
...
REVERSE_HANDLER = DisableAbleCommandHandler(
    ["ges","grs", "reverse"], reverse, pass_args=True, run_async=True
)

PROTECC_HANDLER = DisableAbleCommandHandler(
    ["p", "protecc","pt"], protecc, pass_args=True, run_async=True
)
