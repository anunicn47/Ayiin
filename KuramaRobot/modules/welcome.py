...
import KuramaRobot.modules.sql.welcome_sql as sql
...
from KuramaRobot.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from KuramaRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from KuramaRobot.modules.helper_funcs.msg_types import get_welcome_type
from KuramaRobot.modules.helper_funcs.handlers import MessageHandlerChecker
from KuramaRobot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from KuramaRobot.modules.log_channel import loggable
from KuramaRobot.modules.sql.global_bans_sql import is_user_gbanned
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "A GOD LEVEL DISASTER JUST JOINED", reply_to_message_id=reply
                )
                welcome_log = (
            ...
            if new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "A ANGEL LEVEL DISASTER JUST JOINED",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DRAGONS:
                update.effective_message.reply_text(
                    "A DRAGON LEVEL DISASTER JUST JOINED",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DEMONS:
                update.effective_message.reply_text(
                    "A DEMON LEVEL DISASTER JUST JOINED",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in WOLVES:
                update.effective_message.reply_text(
                    "A WOLVE LEVEL DISASTER JUST JOINED", reply_to_message_id=reply
                )
                continue
            ...
            if new_mem.id == bot.id:
                update.effective_message.reply_text(
                    "Hey {}, I'm Kurama ! Thank you for adding me to {}\n"
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton(
                                    text="📢 Updates", url="https://t.me/Kurama_sumpport_groump"
                                ),
                            ]
                        ],
                    ),
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "A GOD LEVEL DISASTER LEFT", reply_to_message_id=reply
                )
                return
