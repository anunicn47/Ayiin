...
from KuramaRobot import dispatcher, DEV_USERS, telethn, OWNER_ID
from KuramaRobot.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not KuramaRobot.ALLOW_CHATS else "off"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        KuramaRobot.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        KuramaRobot.ALLOW_CHATS = False
    else:
...
def leave(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        update.effective_message.reply_text("I'm going to leave {}, press the button below to confirm".format(chat.title), reply_markup=InlineKeyboardMarkup(kurama_leave_bt))
...
def restart(update: Update, context: CallbackContext):
    ...
    process = subprocess.run("pkill python3 && python3 -m KuramaRobot", shell=True, check=True)
    process.communicate()
